...
# Obtener la ID del nodo (Node ID) y depurar el resultado, preservando el valor de $HOME del usuario actual
node_id=$(sudo -E /usr/local/bin/celestia p2p info --node.store $node_store_path 2>&1)
